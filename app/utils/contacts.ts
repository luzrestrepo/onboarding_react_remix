...
const fakeContacts = {
  ...
  async getAll(): Promise<ContactRecord[]> {
    return sort(Object.values(fakeContacts.records)).by([
      { desc: (contact) => contact.createdAt },
      { asc: (contact) => contact.last }
    ]);
  },
...
export async function getContacts(query?: string | null) {
  ...
  return sort(contacts).by([
    { asc: (contact) => contact.last },
    { asc: (contact) => contact.createdAt }
  ]);
}
